...
- Do not add new feature families to `importer.py`. Add focused modules, keep compatibility wrappers thin, and keep CLI aliases as thin calls into shared helpers. `importer.py` should stay centered on import/rerun/resume flows, run manifests, staging PDF behavior, selected-only semantics, and post-import enrichment coordination.
- Project scaffold/link commands must use focused project navigation refreshes only. Do not trigger broad rebuild side effects such as paper-card normalization, PDF filename repair, or run-link repair unless the user explicitly runs `rebuild`.
