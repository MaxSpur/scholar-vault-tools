...
def _zsh_completion_path() -> Path:
    ...
    return base / ".zfunc" / "_scholar-vault"
...
def _zsh_completion_script() -> str:
    return "\n".join(
        [
            "#compdef scholar-vault",
            "# Generated by `scholar-vault install-zsh-completion`.",
            "# Install as `_scholar-vault` in a directory listed in zsh's fpath.",
            "",
            'eval "$(env _TYPER_COMPLETE_ARGS="${words[1,$CURRENT]}" '
            '_SCHOLAR_VAULT_COMPLETE=complete_zsh scholar-vault)"',
            "",
        ]
    )
...
ZshCompletionPathArg = Annotated[
    Path | None,
    typer.Option(
        "--path",
        exists=False,
        ...
        resolve_path=True,
        help=(
            "Completion file to write. Defaults to ${ZDOTDIR:-~}/.zfunc/_scholar-vault."
        ),
    ),
]
...
def install_zsh_completion_command(path: ZshCompletionPathArg = None) -> None:
    target = path or _zsh_completion_path()
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(_zsh_completion_script(), encoding="utf-8")
    console.print(f"Wrote zsh completion to {escape(str(target))}.")
    console.print("Ensure the parent directory is in zsh's `fpath`, then restart zsh.")
    console.print("For example: `fpath=(~/.zfunc $fpath); autoload -Uz compinit; compinit`.")


@app.command("show-zsh-completion")
def show_zsh_completion_command() -> None:
    typer.echo(_zsh_completion_script(), nl=False)
