...
- `scholar_vault/cli_autopilot.py`: `ask`, `intake`, `improve`, `answer`,
  `session ...`, and `codex ...` command wiring for the user-facing autopilot
  layer.
...
- `scholar_vault/sessions.py`: durable autopilot session, Codex handoff, and
  session-report state under `_sessions/`, `_handoffs/`, and `_reports/`.
- `scholar_vault/autopilot.py`: high-level orchestration over existing query,
  prompt-pack, import, digest, maintenance, lint, eval, rebuild, Bases,
  Obsidian, operation-log, and Codex handoff helpers. It must not scrape
  Scholar or duplicate importer logic.
...
- `scholar_vault/digests.py`: PDF-grounded compile workflow helpers for
  ...
  queues without generating scientific claims.
...
- `codex handoff/run --kind post-import|improve|answer`: writes handoff prompts
  under `_handoffs/`; `run` invokes `codex exec -C <vault> --sandbox
  workspace-write ...`.
...
- Autopilot sessions: `_sessions/<session-id>.yaml` plus
  `_sessions/current.yaml`, with status, question, query, prompt-pack, run,
  blockers, synthesis links, and timestamps for the `ask -> intake -> answer`
  flow.
- Codex handoff prompts: `_handoffs/*.md`, generated by `answer`, `improve`,
  and `codex handoff/run`.
- Session reports: `queries/<slug>/session-report.md` or `_reports/latest.md`.
...
- User-facing acceptance docs: `docs/obsidian-bases-acceptance.md` for manual
  ...
  preferred manual hardening/maintenance command sequence. Detailed lower-level
  CLI workflows live in `docs/advanced-command-reference.md`.
