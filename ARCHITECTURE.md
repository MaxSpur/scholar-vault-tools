...
- `bibtex` / `card-bibtex`: `bibtex` without `--citekey` regenerates `_exports/library.bib`; `bibtex --citekey <key>` and `card-bibtex <key>` render one card for terminal/Obsidian use. One-card and whole-library rendering prefer cached provider BibTeX, then cached CSL JSON, then a card-derived fallback while normalizing keys to vault citekeys, adding vault-local file/abstract/keyword fields, and emitting ASCII/TeX-compatible punctuation and accents.
