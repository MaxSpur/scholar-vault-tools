...
- `enrich-citations`: canonical-card-only DOI, citation, optional abstract metadata enrichment, and missing keyword extraction from attached PDFs. Default behavior enriches DOI/citation data; `--abstracts`, `--only missing-abstract`, or `--refresh-abstracts` switch to abstract enrichment with separate locks and fingerprints, while `--only missing-keywords` retries PDF `Keywords` / `Index Terms` extraction. Progress callbacks emit start/end states plus granular attempt/result/skip events for local scans, provider calls, fallback decisions, and final writes.
