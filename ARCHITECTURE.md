...
- `skills diff` / `skills adopt` / `skills publish` / `skills ui`: safe synchronization for project-local Codex skills between this repository and a vault's `.agents/skills/`. The terminal commands dry-run by default, `adopt` copies a vault-side skill back into the repository source of truth, `publish` copies repository skills into the vault, and vault-only extras are kept unless explicitly archived into `.sync-backups/`.
