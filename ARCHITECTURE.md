...
- `resolve-citation` / `set-metadata`: safe manual citation metadata correction. It updates DOI, authors, year, venue, URL, citation/enrichment status, fingerprints, optional `metadata_lock`, and derived outputs without hand-editing enrichment fields.
